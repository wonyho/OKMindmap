...
function afterDWR_sendNodeCoordMove(id, dx, dy) {
	...
	if(tempNode){
		if( jMap.layoutManager instanceof jTreeLayout || jMap.layoutManager instanceof jHTreeLayout ) {
			tempNode.hgap = parseInt(tempNode.hgap) + parseInt(dy);
			tempNode.vshift = parseInt(tempNode.vshift) + parseInt(dx);
		} else if (tempNode instanceof jPadletNode) {			//KHANG: deal with padlet node's moving
...
function afterDWR_sendNodeRemove(nodeId) {
	var tempNode = jMap.getNodeById(nodeId);
	if(tempNode){
		jMap.fireActionListener("action_NodeUnSelected", tempNode);
...
function afterNodeBranchText(id, text){
	var tempNode = jMap.getNodeById(id);
	if(tempNode) {
		tempNode.setAttributeExecute("branchText", text);
...
function afterNodeNote(id, text){
	var tempNode = jMap.getNodeById(id);
	if(tempNode) {
		tempNode.setNoteExecute(text);
...
function afterNodeFile(id, url){
	var tempNode = jMap.getNodeById(id);
	if(tempNode) {
		tempNode.setFileExecute(url);
...
function afterNodeArrowLink(id, xml){
	if (window.DOMParser) {
		var parser = new DOMParser();
		xmlDoc = parser.parseFromString(xml,"text/xml");
	} else { // Internet Explorer
	...
	if(tempNode && xmlDoc) {
		var nodeEl = xmlDoc.childNodes.item(0);
		var arrowlink = jMap.loadManager.createArrowLink(tempNode, nodeEl);
		tempNode.addArrowLinkExecute(arrowlink);
...
function afterNodeFontSize(id, size){
	var tempNode = jMap.getNodeById(id);
	if(tempNode) {
		tempNode.setFontSizeExecute(size);
	}
}

function afterNodeEdgeColor(id, color, width){
	var tempNode = jMap.getNodeById(id);
	if(tempNode) {
		tempNode.setEdgeColorExecute(color, width);
	}
}

function afterNodeBranch(id, color, width, style){
	var tempNode = jMap.getNodeById(id);
	if(tempNode) {
		tempNode.setBranchExecute(color, width, style);
	}
}
...
function afterDWR_sendNodeInsert(nodeId, data, index2, isLeft2) {
	...
	if(tempNode){
		...
		newNode.setCreator(newNode.userportrait, newNode.userfullname, newNode.created);
...
// OKMChat
function OKMChat_receiveMessages(username, messages, roomnumber, userid, timecreated) {
	if(roomnumber == jMap.cfg.mapId) {
		var isMe = userid == jMap.cfg.userId;
		if(jMap.cfg.userId == '0') {
			isMe = username.indexOf($.cookie('chatGuestID')) != -1;
		}
		OKMChat.receiveMessages(username, messages, isMe, timecreated);
	}
}
function OKMChat_onTyping() {
	$('#rightPanelFolding .typing').removeClass('d-none');
}
function OKMChat_offTyping() {
	$('#rightPanelFolding .typing').addClass('d-none');
}

function afterDWR_Asynchronous() {
	var countdown = 5;
	var dialog = __$('#asynchronousDialog');
	if (!dialog.hasClass('show')) {
		dialog.modal('show');
		$('#asynchronousCountdown').html(countdown);

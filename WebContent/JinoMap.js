...
var ACTIONS = {	ACTION_NEW_NODE : "action_NewNode",				// 새로운 노드가 만들었졌을 때
				...
				ACTION_NODE_FILE : "action_NodeFile",			// 파일이 첨부 되었을 때
				ACTION_NODE_NOTE : "action_NodeNote",			// 노트가 첨부 되었을 때
				...
				ACTION_NODE_BRANCH_TEXT: "action_NodeBranchText",
				ACTION_NODE_ARROWLINK: "action_NodeArrowLink",
				ACTION_NODE_FONTSIZE: "action_NodeFontSize",
				ACTION_NODE_EDGECOLOR: "action_NodeEdgeColor",
				ACTION_NODE_BRANCH: "action_NodeBranch",
				ACTION_PRESENTATION_INSERT: "action_PresentaionInsert", // 슬라이드 추가
				ACTION_PRESENTATION_DELETE: "action_PresentaionDelete", // 슬라이드 삭제
				ACTION_PRESENTATION_UPDATE: "action_PresentaionUpdate", // 하위 토픽 갯수 변경
				ACTION_PRESENTATION_MOVE: "action_PresentaionMove", // 슬라이드 순서 변경
				ACTION_CHAT_SEND_MESSAGE: "action_ChatSendMessage",
				ACTION_CHAT_ON_TYPING: "action_ChatOnTyping",
				ACTION_CHAT_OFF_TYPING: "action_ChatOffTyping",
				ACTION_MAP_RESTRICT_EDITING: "action_MapRestrictEditing",
				ACTION_ASYNCHRONOUS: "action_Asynchronous",
			  };
...
JinoMap = function (work, w, h, mode, contextPath){
	...
	this.mode = mode || 0;	// 0 : 뷰모드 (편집 권한 없음 - 기본)
							// 1 : 일반모드 (모든권한)
	this.canNodeAdd = true;		// 노드 추가 가능 여부
	
	this.Listeners = new Array();	// 리너스 관리	
	
	if(!contextPath) {
		contextPath = this.cfg.contextPath;
	}
...
//	this.movingShape = null;
	...
	// 노드 편집창
	// this.nodeEditorHandle = $('<textarea id="nodeEditor" value="" style="position:absolute;top:0px;left:0px;height:0px;overflow:hidden;display:none; border: 0;border-radius: 5px;outline: none;box-shadow:0 0 0 1px #2196F3;padding:2px 6px;" />').appendTo($('#'+work));
	// this.nodeEditorHandle = $('<div id="nodeEditor" contenteditable style="position:absolute;top:0px;left:0px;overflow:hidden;display:none;background-color: #fff;border-radius: 4px;outline: none;box-shadow:0 0 0 1px #2196F3;padding:2px 6px;"></div>').appendTo($('#'+work));
	this.nodeEditorHandle = $('<textarea id="nodeEditor" value="" style="position:absolute;top:0px;left:0px;width:100px;height:0px;overflow:hidden;display:none;line-height: 1.2;" />').appendTo($('#'+work));
...
//	});
//	
//	if(searchNode.getChildren().length > 0 /*&& !searchNode.folded*/) {
//		if(searchNode.folded) jMap.controller.foldingAction(searchNode);
...
JinoMap.prototype.findNode = function(founds, unfold, node, finds){
	//if(!finds) finds = new Array();
	...
	$.each(founds, function(key, node) {
		if(node.identity == searchNode.id ) match = true;
	})
	...
	var can_find = false;
	if(searchNode.folded){
		if(unfold){
			jMap.controller.foldingAction(searchNode);
			can_find = true;
		}else{
			can_find = false;
		}
	...
	if(searchNode.getChildren().length > 0) {
		...
		if(can_find){
			...
			var cldLength = children.length;
			for(var i = 0; i < cldLength; i++) {
				this.findNode(founds, unfold, children[i], finds);
			}
		}
...
//	this.historyManager.addToHistory(undo, redo);
	...
	var postPasteProcess = function() {
		var pnLength = pasteNodes.length;
		// 저장	
...
JinoMap.prototype.changePositionExecute = function(parent, srcNodes, position, targNode){
	if(jMap.getSelected()) jMap.fireActionListener("action_NodeUnSelected", jMap.getSelected());
	
	var xml = "<paste>";		
	var snLength = srcNodes.length;
...
JinoMap.prototype.createNodeFromText = function(node, text, separate){
	...
	for(var i=0; i < linesLength; i++){
		...
		if(deep > targetDeep + 1) deep = targetDeep + 1;
...
JinoMap.prototype.scale = function(times, frames){
	...
	if (Raphael.svg) {
		...
		var layout = this.layoutManager.type;
 ...
 * 사용자 정보를 가져온다.
 */
JinoMap.prototype.loadUserInfo = function() {
	$.ajax({
		type: 'post',
		async: false,
		url: jMap.cfg.contextPath+'/user/info.do',
		dataType : 'json',
		success: function(data, textStatus, jqXHR){
			jMap.cfg.portrait = data.portrait;
			jMap.cfg.userName = data.name;
			jMap.cfg.userNo = data.userNo;
			jMap.cfg.userId = data.userId;
		},
		error: function(data, status, err) {
		}
    });
...
JinoMap.prototype.setShare = function(share) {
	$.ajax({
		type: 'post',
		async: false,
		url: jMap.cfg.contextPath+'/mindmap/share.do',
		data: {'share': share, 'mapId': jMap.cfg.mapId},
		success: function(data, textStatus, jqXHR){
			jMap.cfg.share = share;
		},
		error: function(data, status, err) {
			alert("setShare : " + status);
		}
    });
}

JinoMap.prototype.setPermission = function(permission) {
	$.ajax({
		type: 'post',
		async: false,
		url: jMap.cfg.contextPath+'/mindmap/permission.do',
		data: {'permission': permission, 'mapId': jMap.cfg.mapId},
		success: function(data, textStatus, jqXHR){
			jMap.cfg.permission = permission;
		},
		error: function(data, status, err) {
			alert("setPermission : " + status);
		}
    });
}

JinoMap.prototype.getUserClassGroup = function() {
	var groups = null;
	
	$.ajax({
		type: 'post',
		async: false,
		url: jMap.cfg.contextPath+'/mindmap/listMyClassGroup.do',
		dataType : 'json',
		success: function(data, textStatus, jqXHR){
			groups = data.groups;
		},
		error: function(data, status, err) {
			alert("getUserClassGroup : " + status);
		}
    });
...
JinoMap.prototype.getUserMaps = function(grpno, search, sort, mapid, page, pagelimit) {
	var maps = null;
	
	$.ajax({
		type: 'post',
		async: false,
		url: jMap.cfg.contextPath+'/mindmap/listMyMap.do',
		data: {'grpno': grpno,
			'search': search,
			'sort': sort,
			'mapId': mapid,
...
JinoMap.prototype.importMap = function(mapid, fromid) {
	var maps = null;
	
	$.ajax({
		type: 'post',
		async: false,
		url: jMap.cfg.contextPath+'/mindmap/import.do',
		data: {
			'mapId': mapid,
			'id': fromid},
		dataType : 'json',
		success: function(data, textStatus, jqXHR){
			//console.log(data);
			document.location.href = jMap.cfg.contextPath+'/map/' + data.mapKey;
		},
		error: function(data, status, err) {
		}
    });
...
JinoMap.prototype.commonAlertAction = function(msg, url){
	// css: https://css-tricks.com/snippets/css/prevent-long-urls-from-breaking-out-of-container/
	// https://kenneth.io/blog/2012/03/04/word-wrapping-hypernation-using-css/
	msg = msg.replace(/(?:\r\n|\r|\n)/g, '<br>');
	
	txt = '<div class="dialog_content" ><div><span>' + msg + '</span></div></div>';

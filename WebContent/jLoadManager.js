		...
		req.onreadystatechange = function () {
			if (req.readyState == 4) {
				...
				if (req.status == 200) {
					...
					xmlStr = xmlStr.split('class="lazy-load"').join('');
...
//	}
	...
	// move to index - jMap.loaded = function() {}
...
jLoadManager.prototype.initChildNodes = function(/*jNode*/parentNode, /*Element*/element){
	if(element.childNodes.length > 0) {
		...
		for(var i = 0; i < cldLength; i++) {
			...
			if(child.nodeType == 1 /*NodeTypes.ELEMENT_NODE*/) {
				var cldNodeName = child.nodeName;
				if( cldNodeName == "node" ) {
				...
				else if( cldNodeName == "edge" ) {
					this.initEdgeAttrs(parentNode, child);
				}
				else if( cldNodeName == "branch" ) {
					this.initBranchAttrs(parentNode, child);
				}
								
				else if( cldNodeName == "richcontent" ) {
					var text = "";
	...
	//					parentNode.setText(child.text);
				...
				else if( cldNodeName == "foreignObject" ) {
					var content = "";
					...
					var ccLength = contentChilds.length;
					for(var foi = 0; foi < ccLength; foi++) {
					...
					if($videoTest && $videoTest.prop("tagName") == "IFRAME") {
						...
						// content = content.replace(' src=', ' class="lazy-load" data-src=');
				...
				else if(cldNodeName == "arrowlink") {
					var arrowlink = this.createArrowLink(parentNode, child);
					parentNode.addArrowLinkExecute(arrowlink);
				}
				
				else if(cldNodeName == "info") {
					...
					if(attr_name == 'iot') {
						var iot_attr = JSON.parse(decodeURI(attr_value));
						if(parentNode['set' + iot_attr.type + 'Execute']) {
							parentNode['set' + iot_attr.type + 'Execute'](iot_attr);
						}
					}

					//alert('load ' + parentNode.plainText + ' ' + attr_name + ': ' + parentNode.attributes[attr_name]);
				}//KHANG
				else if(cldNodeName == "font") {
					var fontSize = child.getAttribute("SIZE");
					parentNode.setFontSizeExecute(fontSize);	
				}
	...
	if(!parentNode.userportrait) {
		parentNode.userportrait = jMap.cfg.defaultUserPortrait;
	}
	parentNode.setCreator(parentNode.userportrait, parentNode.userfullname, parentNode.created);
}
...
jLoadManager.prototype.createArrowLink = function(/*jNode*/node, /*Element*/ element) {
	...
	arrowlink.setText(element.getAttribute("TEXT"));
	arrowlink.setLineWidth(element.getAttribute("LINEWIDTH"));
	arrowlink.style=element.getAttribute("STYLE");
	console.log(element);
	return arrowlink;
}
...
//			}
			if( child.nodeType == 1 /*NodeTypes.ELEMENT_NODE*/) {
				var cldNodeName = child.nodeName;
				var cldNodes = child.childNodes;
				var cldNodesLength = cldNodes.length;
				...
				} else if(cldNodeName == "p") {
					var txt = JinoUtil.trimStr(node.getText());
					if(cldNodesLength > 0){
						if( txt != null && txt != "") txt = txt + "\n" + JinoUtil.trimStr(cldNodes.item(0).nodeValue);
						else txt = JinoUtil.trimStr(cldNodes.item(0).nodeValue);						
					}
...
jLoadManager.prototype.initEdgeAttrs = function(/*jNode*/node, /*Element*/ element){
	...
	for (var i = 0; i < attrLength; i++) {
		...
		if (attrIndex.nodeName) {
			var nodeName = attrIndex.nodeName;
			if (nodeName.toLowerCase() == "color") {
				color = attrIndex.nodeValue;
				continue;
			}
			if (nodeName.toLowerCase() == "width") {
				width = attrIndex.nodeValue;
				continue;
			}
			node.edge[nodeName.toLowerCase()] = attrIndex.nodeValue;
		}
	}
	node.setEdgeColorExecute(color, width);
	node.setBranchExecute(color, null); // without mm_branch table in database
}
jLoadManager.prototype.initBranchAttrs = function(/*jNode*/node, /*Element*/ element){
	var attr = element.attributes;
	var color = null;
	var width = null;
	var style = null;
	...
	for (var i = 0; i < attrLength; i++) {
		...
		if (attrIndex.nodeName) {
			...
			if (nodeName.toLowerCase() == "style"
				&& jMap.layoutManager.type == "jMindMapLayout") { //
				// style = attr[i].nodeValue;
	...
	node.setBranchExecute(color, width, style);
}
...
jLoadManager.prototype.initNodeAttrs = function(/*jNode*/node, /*Element*/ element){
	...
	for (var i = 0; i < attr.length; i++) {
		...
		if (attrIndex.nodeName) {
			...
			if (nodeName.toLowerCase() == "background_color") {
				node.setBackgroundColorExecute(attrIndex.nodeValue);
				continue;
			}
			if (nodeName.toLowerCase() == "color") {
				node.setTextColorExecute(attrIndex.nodeValue);
				continue;
			}
			if (nodeName.toLowerCase() == "link") {
				node.setHyperlinkExecute(attrIndex.nodeValue);
				continue;
			}
			if (nodeName.toLowerCase() == "file") {
				node.setFileExecute(attrIndex.nodeValue);
				continue;
			}
			if (nodeName.toLowerCase() == "note") {
				node.setNoteExecute(attrIndex.nodeValue);
				continue;
			}	
			if (nodeName.toLowerCase() == "folded") {
				node.folded = (attrIndex.nodeValue == "true") ? true : false;				
				continue;
			}
			if (nodeName.toLowerCase() == "user_portrait") {
				node.userportrait = attrIndex.nodeValue;				
				continue;
			}
			if (nodeName.toLowerCase() == "user_fullname") {
				node.userfullname = attrIndex.nodeValue;				
				continue;
			}
			node[nodeName.toLowerCase()] = attrIndex.nodeValue;
...
jLoadManager.prototype.pasteNode = function(node, xml, index, position, revivePosition) {
	...
	var tagName = docNode.tagName;
	while (docNode.nodeType != 1 && (tagName != 'clipboard' || tagName != 'paste' || tagName != 'okm' || tagName != 'node')) {
	...
	var newNodes = [];
	var nodeLength = nodeElements.length;
	for(var i = 0; i < nodeLength; i++) {
...
jLoadManager.prototype.updateMoodleIconLoading = function(el) {
	this.imageLoading.push(el);
}
...
jLoadManager.prototype.lazyLoading = function(node){
	...
	$.ajax({
		...
		success: function(data, textStatus, jqXHR) {
			...
			var nodeLength = loadingNodes.length; 
			if(nodeLength > 0) {

...
<head>
	...
	<title>
		<spring:message code='message.group.update.title' />
	</title>
	...
	<div class="container-fluid p-3" style="max-width: 500px; min-height: 450px;">
		...
		<form id="frm_group" action="${pageContext.request.contextPath}/group/update.do" method="post">
			...
			<div class="form-group">
				...
				<input autofocus required class="form-control" type="text" name="name" id="name" value="<c:out value='${data.group.name }' />" />
			</div>
			...
			<div class="form-group">
				...
				<select required name="parent" class="form-control">
					<c:forEach items="${data.categories }" var="category">
						<option value="<c:out value='${category.id}' />" <c:if test="${category.id == data.group.category.parentId }">selected</c:if>
							>
			...
			<div class="form-group">
				...
				<select id="policy" required name="policy" class="form-control" onChange="setPolicy(this.options[this.selectedIndex].value)">
					<c:forEach items="${data.policies}" var="policy">
						<option value="<c:out value='${policy.shortName}' />" <c:if test="${policy.shortName == data.group.policy.shortName }">selected</c:if>
							>
			...
			<div class="form-group">
				...
				<textarea name="summary" class="form-control" rows="4"><c:out value="${data.group.summary }" /></textarea>
			</div>
			...
			<c:if test="${data.tiers != null}">
				<div class="form-group">
					<label>Plan</label>
						<div>
							<input ${data.tier_group == 0 ? 'checked':''} type="radio" name="tier_id" id="tier_id_0" value="0">
							<label for="tier_id_0">(None)</label>
						</div>
						<c:forEach var="tier" items="${data.tiers}">
							<c:if test="${tier.activated}">
								<div>
									<input ${data.tier_group == tier.id ? 'checked':''} type="radio" name="tier_id" id="tier_id_${tier.id}" value="${tier.id}">

...
<head>
    ...
    <script type="text/javascript">
		var addDevice = function(){
			$.ajax({
			      type: 'POST',
			      url: parent.jMap.cfg.contextPath + '/iot/add.do',
			      data: {name : $("#name").val(), type: $("#mains").val(), confirm : 1},
            ...
            <form id="frm_confirm">
				...
				<div class="form-group">
                    ...
                    <select class="custom-select col" id="mains">
                    	<option value="0" selected>Select a motherboard</option>
                    	<c:forEach var="main" items="${data.modules }">
                    		<option value="${main.getId()}">${main.getName()}</option>
                    	</c:forEach>
				  	</select>
                </div>

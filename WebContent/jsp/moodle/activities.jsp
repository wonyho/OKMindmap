...
<script type="text/javascript">
	...
	function findSection(node) {
		if(node == null || node.isRootNode()) return "0";
		if(node.attributes.moodleCourseSection != undefined) return node.attributes.moodleCourseSection;
		
		return findSection(node.getParent());
	}

	function checkSubmit(frm){
		if(addModule != null){
			if(addModule.name == 'fn'){
				parent.insertActivityAction("${pageContext.request.contextPath}/moodle/launch.do?url=" + addModule.link);
			}else {
				var node = parent.jMap.getSelected();
				
				parent.location.href = addModule.link + "&mapid=<%=mapId%>&mapkey=<%=mapKey%>&section=" + findSection(node);
	...
	<tbody>
		<tr>
			<td style="border-right: 1px solid #ddd;width: 60%;padding-right: 15px;">
				<div class="moodle-group-title"><spring:message code='message.moodle.activities'/></div>
			...
			<td style="padding-left: 15px;">
				<div class="moodle-group-title"><spring:message code='message.moodle.course_administration'/></div>
...
<div class="actions">
	<input type="submit" class="create_btn disable" id="create_btn" value="<spring:message code='common.confirm' />" />
</div>
</div>
</form>

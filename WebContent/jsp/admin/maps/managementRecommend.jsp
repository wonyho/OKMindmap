	...
	<script type="text/javascript">
		...
		function formatDate(){
			$('.format-date').each(function() {
				...
				if(data != '') {
					var date = new Date();
					var pattern = $(this).data('pattern');	// yyyy-MM-dd HH:mm
					date.setTime(data);
					
					pattern = pattern.replace("yyyy", date.getFullYear());
					pattern = pattern.replace("MM", (date.getMonth() < 9 ? "0":"") + (date.getMonth() + 1));
					pattern = pattern.replace("dd", (date.getDate() < 10 ? "0":"") + date.getDate());
					pattern = pattern.replace("HH", (date.getHours() < 10 ? "0":"") + date.getHours());
					pattern = pattern.replace("mm", (date.getMinutes() < 10 ? "0":"") + date.getMinutes());
		...
		<div class="table_box">
				...
				<c:forEach var="map" items="${data.maps}">
					<tr height="26px">
						...
						<td align="center">
							<span class="format-date" data-date="<c:out value="${map.created}"/>" data-pattern="yyyy-MM-dd HH:mm" ></span>
						</td>
						<td align="center">
							<span class="format-date" data-date="<c:out value="${map.added}"/>" data-pattern="yyyy-MM-dd HH:mm" ></span>
						</td>
						<td align="center">

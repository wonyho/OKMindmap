...
<head>
	...
	<script type="text/javascript">
		...
		function compileUser(data) {			
			...
			var account_level = (userdata[4] == null || userdata[4] == "")? "" : "&account_level="+userdata[4];
			
			$.ajax({
				type: 'post',
				async: false,
				url: '${pageContext.request.contextPath}/mindmap/admin/users/useradd.do?adduser=1' +
						username + lastname + firstname + email + account_level,				
				success: function(data, textStatus, jqXHR) {
					if(jqXHR.responseText != "") {
						try {
							var user = JSON.parse(jqXHR.responseText);
	...
	<div id="content">
		<div class="adduser">
			...
			<form id="frm_adduser" name="frm_adduser" action="${pageContext.request.contextPath}/mindmap/admin/users/useradd.do" method="post">
				...
				<tr>
					<td align="right" class="r1">Account level</td>
					<td>
						<c:forEach var="tier" items="${data.tiers}">
							<c:if test="${tier.activated}">
								<div>
									<input type="radio" name="account_level" id="account_level_${tier.id}" value="${tier.id}">
									<label for="account_level_${tier.id}">${tier.name}</label>
								</div>
							</c:if>
						</c:forEach>
					</td>

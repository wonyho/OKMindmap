...
<head>
	...
	<script type="text/javascript">
		...
		var editProfile = function(userid) {
			// $.modal('<iframe src="${pageContext.request.contextPath}/user/update.do?userid='+userid+'" frameborder="0" allowtransparency="true" width="500"  height="892" scrolling="no"></iframe>', {
			// 		overlayId: 'okm-overlay',
			// 		containerId: 'editProfile-container',
			// 		dataId: 'editProfile-data'});
			window.location.href = "${pageContext.request.contextPath}/mindmap/admin/users/useredit.do?userid=" + userid;
		}
	</script>
			...
			<div class="table_box">
				...
				<c:forEach var="user" items="${data.users}">
					<tr>
						...
						<td align="center"><a href="${pageContext.request.contextPath}/mindmap/admin/users/useredit.do?userid=${user.id}"><c:out value="${user.lastname}"/><c:out value="${user.firstname}"/></a></td>

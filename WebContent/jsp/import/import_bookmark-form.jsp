...
<head>
	...
	<title>
		<spring:message code='message.import.bookmark.upload' />
	</title>
	...
	<script type="text/javascript">
		var FILEUPLOAD_API_URL = "${pageContext.request.contextPath}/mindmap/importBookmark.do";
		...
		function importBookmark() {
			...
			xhr.onreadystatechange = function () {
				if (xhr.readyState === 4 && xhr.status === 200) {
					$('#fileupload').removeClass('onupload');
					if(xhr.response.length < 30000) {
						parent.JinoUtil.BookmarkCallback(xhr.response);
						parent.JinoUtil.closeDialog();
					} else {
						alert('<spring:message code="alert.bookmark" />');
					}
				}
			}
			xhr.send(formData);
		...
		$(document).ready(function () {
			...
			$("#frm_confirm").submit(function (event) {
				...
				if (document.getElementById('fileupload_input').files.length) importBookmark();
			});
		});
	</script>
</head>
		...
		<div class="py-4 text-center">
			<div id="fileupload" class="mx-auto fileupload-thumbnail position-relative d-inline-block">
				...
				<div id="file_preview" class="img-thumbnail text-center p-4 d-none position-relative" style="min-width: 300px;">
					...
					<span id="file_type" class="position-absolute text-uppercase font-weight-bold text-white rounded">HTML</span>
				...
				<input id="fileupload_input" type="file" accept=".html" class="position-absolute top-0 left-0 w-100 h-100" />
			</div>
			<p class="font-italic">
				<spring:message code='message.import.bookmark.default' />
			</p>
		</div>

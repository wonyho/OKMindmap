...
<head>
    ...
    <title>
        <spring:message code='menu.nodeBGColorAction' />
    </title>
    ...
    <script type="text/javascript">
        var picker = null;
        function nodeBGColorAction() {
            var color = $('#color').val();
            if (color == '') color = '#ffffff';

            var selectedNodes = parent.jMap.getSelecteds();
            for (var i = 0; i < selectedNodes.length; i++) {
                selectedNodes[i].setBackgroundColor(color);
            }
        ...
        $(document).ready(function () {
            ...
            if (node) {
                $('#color').val(node.getBackgroundColor());
            }

            picker = $("#color").spectrum({
                allowEmpty: true,
                color: (node ? node.getBackgroundColor() : '#ffffff'),
                showInput: true,

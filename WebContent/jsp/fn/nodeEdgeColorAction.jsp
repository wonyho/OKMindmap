...
<head>
    ...
    <title>
        <spring:message code='menu.nodeEdgeColorAction' />
    </title>
    ...
    <script type="text/javascript">
        var picker = null;
        function nodeEdgeColorAction() {
            var color = $('#color').val();
            if (color == '') color = '#000000';

            var selectedNodes = parent.jMap.getSelecteds();
            for (var i = 0; i < selectedNodes.length; i++) {
                selectedNodes[i].setEdgeColor(color);
                selectedNodes[i].setBranch(color);
            }
        ...
        $(document).ready(function () {
            ...
            if (node) {
                $('#color').val(node.getEdgeColor());
            }

            picker = $("#color").spectrum({
                allowEmpty: true,
                color: (node ? node.getEdgeColor() : '#000000'),
                showInput: true,

...
<body class="bg-gray-200 p-3">
	<div class="d-flex justify-content-center align-items-center vh-100">
		<div class="text-center py-5 px-3 bg-white shadow-lg rounded-lg w-100" style="min-height: 200px; max-width: 600px;">
			<img src="${pageContext.request.contextPath}/theme/dist/images/searching.svg" width="80px;" class="mb-5">
			<h2 class="tit v1 h4"></h2>
			<div class="mt-4">
				<button class="btn btn-primary btn-min-w" id="cancleBackBtn" type="button"><spring:message code="button.back" /></button>
				<button class="btn btn btn-outline-primary btn-min-w" id="toMain_btn" type="button"><spring:message code="error.gomain"/></button>
			</div>
		</div>
	</div>
	...
	<script type="text/javascript">
		$(document).ready(function () {
			...
			console.log('<c:out value="${data.message}"/>');
			console.log(sessionCheckUrl); */

			if (message != '') {
				message = message.replace('&lt;', '<').replace('&gt;', '>');
				$("h2.v1").html(message);
			} else {
				$("h2.v1").html('<spring:message code="error.pagenotfound" />');
			}

			var fnPageMove = function (obj_type) {
				if (AppUtil.getNativeOS() != "") {
					...
					if (obj_type === 0) {
						...
						location.href = sessionCheckUrl;
					}
				}
			};

			if (ISMOBILE || supportsTouch || AppUtil.getNativeOS() != "") {
				$("#cancleBackBtn").hide();
			};

			$("#cancleBackBtn").on("click touchstart", function () {
				fnPageMove(0);
			});
			// $("#toMain_logo").on("click touchstart", function(){
			// 	fnPageMove(1);
			// });

...
jSunburstController.prototype.startNodeEdit = function(node) {
	...
	oInput.style.fontSize = jMap.cfg.nodeFontSizes[3] * this.map.cfg.scale + "px";
	oInput.style.textAlign = 'left';

	// var width = (node.outerRadius - node.innerRadius) * jMap.cfg.scale;
	// var height = 30 * jMap.cfg.scale;
	var width = node.popover.container.width() * jMap.cfg.scale;
	var height = node.popover.container.height() * jMap.cfg.scale;
	var left = RAPHAEL.getSize().width / 2;
	var top = (RAPHAEL.getSize().height / 2);
...
jSunburstLayout = function(map) {
	...
	// self.radius = Math.max((Math.min(work.offsetWidth / 2, work.offsetHeight / 2)) - 100, 350);
	...
	/*if(!(ISMOBILE || supportsTouch)) {
		self.map.extendMenu = $('<li class="menu" title="마디수정" onclick="editNodeAction()" style="cursor: pointer;"><img src="'+jMap.cfg.contextPath+'/menu/icons/node_decorate_bak.png"/></li>').appendTo('#edit-menu #header ul');
	}*/
};

...
jPadletLayout.prototype.checkLocation = function(node, sibling) {
	...
	if(!node.isRootNode() && (node.attributes['padlet_x'] == 0 || node.attributes['padlet_y'] == 0)) {
		if (sibling) {
			var pl = {
				x: sibling.attributes.padlet_x,
				y: sibling.attributes.padlet_y
			};
			var sz = sibling.getSize();
			
			node.attributes['padlet_x'] = '' + (parseFloat(pl.x) + 0.0); 
			node.attributes['padlet_y'] = '' + (parseFloat(pl.y) + parseFloat(sz.height) + 20.0);
	
			node.setLocation(node.attributes['padlet_x'], node.attributes['padlet_y']);
			
		} else if (node.getParent()) { 	// 초기 위치
			var pl = {
				x: node.getParent().attributes.padlet_x,
				y: node.getParent().attributes.padlet_y
			};
			var sz = node.getParent().getSize();
			...
			node.attributes['padlet_x'] = '' + (parseFloat(pl.x) + parseFloat(sz.width) + 20.0); 
			node.attributes['padlet_y'] = '' + (parseFloat(pl.y) + 0.0);
	...
	if (node.folded == "false" || node.folded == false) {
		...
		if (children != null && children.length > 0) {
			for (var i = 0; i < children.length; i++) {
				this.checkLocation(children[i], i>0 ? children[i-1]:null);
			}
		}
	}
}

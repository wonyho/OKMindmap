...
//	}
	...
	var _wear = function(node) {
		...
		if(node.theme) {
			...
			node.theme.branch && node.setBranchExecute(node.theme.branch, node.theme.branch_width, null);
		} else {
			...
			node.setBranchExecute(node.getParent().getBranchColor(), jMap.cfg.branchDefalutWidth, null);
		}
	}

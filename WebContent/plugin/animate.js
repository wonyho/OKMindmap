...
ScaleAnimate = (function () {
	...
	var createNavigation = function() {
		...
		if(jMap.getLayoutManager().type == "jSunburstLayout") {
			$naviContainer.css("pointer-events", "none");
			$nextArea.css("pointer-events", "visible");
			$prevArea.css("pointer-events", "visible");
			$naviNext.css("pointer-events", "visible");
			$naviPrev.css("pointer-events", "visible");
		}
...
//		ScaleAnimate.rotate(tempNode, -90, cx, cy);
	...
	ScaleAnimate.endShowMode = function() {
		...
		if(showPresentation) {
			window.location.href = jMap.cfg.contextPath + '/map/' + jMap.cfg.mapKey;
		} else {
			$buttonBox.remove();

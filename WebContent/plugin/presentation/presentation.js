...
//				var $toolsForm = $(e.target);
				...
				$toolsForm.css({
					left: tx+x*slideBoxScale,
					top: ty+y*slideBoxScale
				});
...
//				var $toolsForm = $(e.target);
		...
		var toolsFormResizeStop = function(event, ui) {
			...
			$slide.trigger("mousedown.slidedown");
		}
		...
		var toolsFormResize = function(event, ui) {
			...
			$toolsForm.css({
				left: ( ( tfX*slideBoxScale + $slide.outerWidth(true) * (1 - slideScale.scaleX)*slideBoxScale / 2 ) / scale) + (o.left - p.left) - 4,
				top: ( ( tfY*slideBoxScale + $slide.outerHeight(true) * (1 - slideScale.scaleY)*slideBoxScale / 2 )  / scale) + (o.top - p.top) - 5
			});
		}
...
//			$buttonBox.append($exporthtmlbtn);
		...
		if(isDesign) {
			$closebtn.click(function() {
				Presentation.saveSlide(function() {
					parent.$("#presentation").addClass("d-none");
				});
			});
		} else {
			$closebtn.click(Presentation.close);
		...
		if(cfg.effect == "Dynamic" || cfg.effect == "Box") {
			var $jumpTobtn = $(btn('jmpress-jumpto-btn', 'Jump to'));
			$jumpTobtn.click(function(){
				$('#jt-panel').addClass('show');
			});
			$buttonBox.append($jumpTobtn);
		}
		
		$buttonBox.appendTo($(document.body));
	}

	var createJumptoPanel = function() {
		var has_opacity = parent.jMap.cfg.default_menu_opacity == 'true' ? 'jmpress-jumpto-transparent-panel':'';
		var jumptoPanel = '<div id="jt-panel" class="jmpress-jumpto-panel ' + has_opacity + '">';
		jumptoPanel += '<div class="jt-header">';
		jumptoPanel += '	<div><b>Jump to slide</b></div>';
		jumptoPanel += '	<button class="gh-btn" onclick="$(\'#jt-panel\').removeClass(\'show\')">';
		jumptoPanel += '		<img src="'+jMap.cfg.contextPath+'/theme/dist/images/icons/arrow-left.svg" width="18px">';
		jumptoPanel += '	</button>';
		jumptoPanel += '</div>';
		jumptoPanel += '<div class="jt-content" id="jt-content">';
		jumptoPanel += '</div>';
		jumptoPanel += '</div>';
		$(document.body).append(jumptoPanel);
		setTimeout(function(){
			$('#jt-panel').bind('touchmove', function (e) {
				e.stopPropagation(); 
			});
		}, 200);
	}

	var addJumptoPanelItem = function(id, title) {
		if($('.jt-'+id).length == 0) {
			if(!title || title == '') {
				title = '<i style="color: #666;">(Empty)</i>';
			}
			var _item = '<div class="jt-item">';
			
			_item += '<div class="jt-tit">';
			_item += ($('.jt-item').length + 1)  + '. ' + title;
			_item += '</div>';

			_item += '<div class="jt-icon jt-'+id+'" onclick="Presentation.jumpTo(\'#'+id+'\')">';
			_item += '	<img src="'+jMap.cfg.contextPath+'/theme/dist/images/icons/chevron-right.svg" width="18px">';
			_item += '</div>';

			_item += '</div>';
			$('#jt-content').append(_item);
		}
	}
	...
	Presentation.start = function (auto_start) {
		...
		$box.hide();
		JinoUtil.waitingDialog('프리젠테이션을 준비중 입니다.', '잠시만 기다려 주세요');
		...
		$content_fade.fadeIn(cfg.overlayspeed, function() {
			...
			if($(window).width() < $(window).height()) {
				if($(window).width() < 900) slideBoxScale = $(window).width()/950;
			} else {
				if($(window).height() < 700) slideBoxScale = $(window).height()/750;
			}
			if(slideBoxScale < 1) {
				$('#jmpress').parent().css('transform', 'scale('+slideBoxScale+')');
			}
		});
	};
	
	Presentation.close = function () {
		jMap.cfg.presentationMode = false;
		...
		// nvhoang
		// orignalHref = orignalHref.replace('/show/map/', '/map/');
		// if(orignalHref.indexOf('?') == -1) {
		// 	window.location.href = orignalHref.split('#')[0];
		// } else {
		// 	window.location.href = orignalHref.split('?')[0];
		// }
		
		///////////////////////////////////////////////////////////
		// hide presentation without redirect url
		// jMap.work.focus();
		...
		// 		if($content_fade) $content_fade.remove();
		// 		if($buttonBox) $buttonBox.remove();
		// 		if($toolsForm) $toolsForm.remove();
		...
		if(parent.showPresentation) {
			parent.location.href = jMap.cfg.contextPath + '/map/' + jMap.cfg.mapKey;
		} else {
			parent.presentationClose();
		}
	}
	...
	Presentation.exportHtml = function () {	
		...
		text += '<link rel="stylesheet" href="'+jMap.cfg.contextPath+'/css/presentation.css" type="text/css" media="screen">';
		...
		text += '<script src="'+jMap.cfg.contextPath+'/extends/jmpress.all.js" type="text/javascript" charset="utf-8"></script>';
		text += '</head><body>';
		text += startCode;
	...
	Presentation.jumpTo = function (id) {	
		if($(id).parent().hasClass('step')) {
			// Box
			$('#jmpress').jmpress('goTo', '#' + $(id).parent().attr('id'));	
		} else if($(id).parent().parent().hasClass('step')) {
			//Dynamic
			$('#jmpress').jmpress('goTo', '#' + $(id).parent().parent().attr('id'));	
		}
	}
...
//			$('.slide').bind('mouseup', slideup);
			var slidedown = function(e) {
				...
				$toolsForm.css({
					left: (( ( tfX*slideBoxScale + $(this).outerWidth(true) * (1 - slideScale.scaleX)*slideBoxScale / 2 ) / scale) + (o.left - p.left) - 6),
					top: (( ( tfY*slideBoxScale + $(this).outerHeight(true) * (1 - slideScale.scaleY)*slideBoxScale / 2 )  / scale) + (o.top - p.top) - 6),
					width: (($(this).outerWidth(true) / scale) * slideScale.scaleX*slideBoxScale + 12),
					height: (($(this).outerHeight(true) / scale) * slideScale.scaleY*slideBoxScale + 12),
					'transform': 'rotate('+radian+'deg)',
					'-ms-transform': 'rotate('+radian+'deg)',
					'-webkit-transform': 'rotate('+radian+'deg)',
					'-o-transform': 'rotate('+radian+'deg)',
	...
	Presentation.saveSlide = function(callback) {
		var elements = EditorManager.getPresentationElements();
		var data = Array();
		for(var i = 0; i < elements.length; ++i) {
			var el = elements[i];
			var d = { nodeid: el.node.getID(),
...
//			}
			...
			if(!$s || $s.length == 0) {
				addJumptoPanelItem(selectionId, $('<div>'+(title || content)+'</div>').text());
...
//				str = str.replace(new RegExp('style=', 'g'), 'style-disabled=');
		...
		for(var pos = 0; pos < EditorManager.getSequenceSize(); pos++) {
			...
			if(showDepths < 2) {
				appendSlide('ss'+pos, '<div class="slide_sentence_1">' + text + '</div>', '', ptid);
				if(node.img){
					appendSlide('ss'+pos, '<img class="slide_image_1" src="' + node.imgInfo.href + '" />', '', ptid);
				}
				if(node.foreignObjEl) {
					appendSlide('ss'+pos, '<div class="slide_foreignobj_1 foreignobjfix">'+node.foreignObjEl.plainHtml+'</div>', text, ptid);
				}
			} else {
				appendSlide('ss'+pos, null, text, ptid);
				var contentChildren = function(n, currentDepth) {
					...
					for(var i = 0; i < children.length; i++) {
						...
						if(child.getText() != ''){
							...
							appendSlide('ss'+pos, '<div class="slide_sentence_'+currentDepth+'">' + childText + '</div>', text, ptid);
						}													
						if(child.img){
							//appendSlide('ss'+pos, '<img class="slide_image_'+currentDepth+'" src="' + child.imgInfo.href + '" width="'+ 600 + 'px" height="' + 400 + 'px" />', text);						
							appendSlide('ss'+pos, '<img class="slide_image_'+currentDepth+'" src="' + child.imgInfo.href + '" />', text, ptid);
						}
						if(child.foreignObjEl) {
							...
							appendSlide('ss'+pos, '<div class="slide_foreignobj_'+currentDepth+' foreignobjfix">'+getForeignObjContent(child)+'</div>', text, ptid);
						}

 ...
 */
///////////////////////////////////////////////////////////////////////////////
///////////////////    jPartitionNodeController    //////////////////////
///////////////////////////////////////////////////////////////////////////////
jPartitionNodeController = function() {
	jPartitionNodeController.superclass.call(this);
};
extend(jPartitionNodeController, jNodeController);

jPartitionNodeController.prototype.type = "jPartitionNodeController";

jPartitionNodeController.prototype.dblclick = function(e) {
	...
	if(node.children.length && jMap.arcTweenNode.id != node.id){
		node.zoomExecute();
	}else{
...
jPartitionNodeController.prototype.mouseenter = function(e) {
	this.node.showPopover();
};

jPartitionNodeController.prototype.mouseleave = function(e) {
...
jPartitionNodeController.prototype.popoverMouseleave = function(e) {
...
///////////////////////////////////////////////////////////////////////////////
////////////////// jPartitionNodeControllerGuest    /////////////////////
///////////////////////////////////////////////////////////////////////////////
jPartitionNodeControllerGuest = function() {
	jPartitionNodeControllerGuest.superclass.call(this);
};
extend(jPartitionNodeControllerGuest, jNodeControllerGuest);

jPartitionNodeControllerGuest.prototype.type = "jPartitionNodeControllerGuest";

jPartitionNodeControllerGuest.prototype.dblclick = function(e) {
	var originalEvent;
	if (!e) var e = window.event;
	originalEvent = e.originalEvent.originalEvent || e.originalEvent || e;
	if (originalEvent.preventDefault)
	...
	if(this.node.children.length && jMap.arcTweenNode.id != this.node.id){
...
jPartitionNodeControllerGuest.prototype.mouseenter = function(e) {
	this.node.showPopover();
};

jPartitionNodeControllerGuest.prototype.mouseleave = function(e) {
	var id = 'data-popover-id';
	var el = e.toElement || e.relatedTarget;
	if ($(el).closest('.jpopover').parent().attr(id) != this.node.popover.container.attr(id)) {
		this.node.hidePopover();
...
jPartitionNodeControllerGuest.prototype.popoverMouseleave = function(e) {
	var id = $(this).parent().attr('data-popover-id');
	if (jMap.nodes[id]) {
		jMap.nodes[id].hidePopover();
	}
...
jPartitionNode = function(param) {
	...
	jPartitionNode.superclass.call(this, parentNode, text, id, index, position);
};

extend(jPartitionNode, jNode);

jPartitionNode.prototype.type = "jPartitionNode";

jPartitionNode.prototype.initElements = function() {
...
jPartitionNode.prototype.create = function() {
...
var custom_partition_touchtime = 0;
jPartitionNode.prototype.addEventController = function(c) {
	c = jMap.mode ? new jPartitionNodeController() : new jPartitionNodeControllerGuest();
	this.controller = c;
	...
	this.popover.container.children('.jpopover').on('vclick', function() {
		if (custom_partition_touchtime == 0) {
			// set first click
			custom_partition_touchtime = new Date().getTime();
		} else {
			// compare first click to this click and see if they occurred within double click threshold
			if (((new Date().getTime()) - custom_partition_touchtime) < 400) {
				custom_partition_touchtime = 0;
...
jPartitionNode.prototype.setTextExecute = function(text) {
	this.plainText = text;
	this.text.attr({
		text : text.split('\n')[0]
	});
...
jPartitionNode.prototype.setHyperlink = function(url) {
...
jPartitionNode.prototype.setHyperlinkExecute = function(url) {
	if (url == null || url == "") {
		if (this.hyperlink) {
			this.hyperlink = null;
...
jPartitionNode.prototype.setFile = function(url){
	if(!jMap.saveAction.isAlive()) {
		return null;
	}
...
jPartitionNode.prototype.setFileExecute = function(url){
	if (url == null || url == "") {
		if (this.file){
...
jPartitionNode.prototype.setNote = function(txt){
	if(!jMap.saveAction.isAlive()) {
		return null;
	}
...
jPartitionNode.prototype.setNoteExecute = function(txt){
	if (txt == null || txt == "") {
		if (this.note){
...
jPartitionNode.prototype.setImage = function(url, width, height) {
	if(!jMap.saveAction.isAlive()) {
		return null;
	}
...
jPartitionNode.prototype.setImageExecute = function(url, width, height, _callback) {
	if (url == null || url == "") {
		if (this.img) {
			this.img = null;
			this.imgInfo = {};
...
jPartitionNode.prototype.imageResize = function(width, height) {
	if(!jMap.saveAction.isAlive()) {
		return null;
	}
...
jPartitionNode.prototype.imageResizeExecute = function(width, height) {
...
jPartitionNode.prototype.setForeignObject = function(html, width, height) {
	if(!jMap.saveAction.isAlive()) {
		return null;
	}
...
jPartitionNode.prototype.setForeignObjectExecute = function(html, width, height) {
...
jPartitionNode.prototype.foreignObjectResize = function(width, height) {
	if(!jMap.saveAction.isAlive()) {
		return null;
	}
...
jPartitionNode.prototype.foreignObjectResizeExecute = function(width, height) {
...
jPartitionNode.prototype.setEmbedVideo = function(code) {
...
jPartitionNode.prototype.setVideo = function(playUrl, width, height) {
...
jPartitionNode.prototype.setYoutubeVideo = function(playUrl, width, height) {
	var defaultSize = jMap.cfg.default_video_size;
...
jPartitionNode.prototype.zoomExecute = function(duration) {
	...
	var kx = (node.y ? layout.width - 40 : layout.width) / (1 - node.y);
    var ky = layout.height / node.dx;
    layout.x.domain([node.y, 1]).range([node.y ? 40 : 0, layout.width]);
    layout.y.domain([node.x, node.x + node.dx]);

	var cell = d3.selectAll('g[data-node-type="' + jMap.cfg.nodeStyle + '"]')
		.transition()
        .duration(duration)
        .attr('transform', function(d) { 
			if(!d) return null;
			return 'translate(' + (centerLocation.x + layout.x(d.y)) + ',' + (centerLocation.y + layout.y(d.x)) + ')'; 
		});

    cell.select('rect')
        .attr('width', node.dy * kx)
        .attr('height', function(d) {
			if(!d) return null; 
			jMap.nodes[d.id].pWidth = d.dy * kx;
			jMap.nodes[d.id].pHeight = d.dx * ky;
			return d.dx * ky; 
		});
	...
	cell.select('text')
		.style('opacity', function(d){
			...
			return (d.dx * ky) < (lines.length * jMap.cfg.nodeFontSizes[1]) ? 0:1;
		})
        .attr('transform', function(d) {
			if(!d) return null; 
			setTimeout(function() {
			...
			return 'translate(' + layout.padding + ',' + (d.dx * ky) / 2 + ')';
		});
...
jPartitionNode.prototype.getChildren = function() {
	return this.children || [];
};

jPartitionNode.prototype.screenFocus = function() {
	this.zoomExecute();
};

jPartitionNode.prototype.showPopover = function() {
...
jPartitionNode.prototype.hidePopover = function() {
	this.popover.container.removeClass('active');
};

jPartitionNode.prototype.getPopoverLocation = function() {
...
jPartitionNode.prototype.getPartitionTreeData = function() {
	var treeData = {};
	treeData.id = this.id;
...
jPartitionNode.prototype.getText = function() {
	return this.plainText;
};

jPartitionNode.prototype.computedTextExecute = function() {
	var node = this;
...
jPartitionNode.prototype.toXML = function(alone) {
...
jPartitionNode.prototype.setFolding = function(folded) {
	if (jMap.jDebug) console.log('setFolding');
};
jPartitionNode.prototype.hideChildren = function(node) {
	if (jMap.jDebug) console.log('hideChildren');
};
jPartitionNode.prototype.relativeCoordinate = function(dx, dy) {
	if (jMap.jDebug) console.log('relativeCoordinate');
};
jPartitionNode.prototype.relativeCoordinateExecute = function(dx, dy) {
	if (jMap.jDebug) console.log('relativeCoordinateExecute');
};

jPartitionNode.prototype.getLocation = function(){
	var popover = this.getPopoverLocation();
	return {x:popover.top, y: popover.left};
}

jPartitionNode.prototype.setFontSizeExecute = function(size) {
	//
};
jPartitionNode.prototype.setEdgeColorExecute = function(color, width){
	if (color) {

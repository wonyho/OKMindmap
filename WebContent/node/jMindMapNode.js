...
jMindMapNode.prototype.translate = function(x, y){
	...
	this.file && this.file.translate(x, y);
	this.note && this.note.translate(x, y);
	
	this.moodleIcon && this.moodleIcon.translate(x, y);
	this.score.translate(x, y);
...
//	}
	...
	if(jMap.layoutManager.type == "jTableLayout" ||
			jMap.layoutManager.type == "jTreeLayout" ||
			jMap.layoutManager.type == "jHTreeLayout") return false;
...
jMindMapNode.prototype.setAttribute = function(name, value) {
	if(!jMap.saveAction.isAlive()) {
		return null;
	}
	...
	this.setAttributeExecute(name, value);
...
jMindMapNode.prototype.setAttributeExecute = function(name, value) {
...
jMindMapNode.prototype.setFile = function(url){
	if(!jMap.saveAction.isAlive()) {
		return null;
	}
...
jMindMapNode.prototype.setFileExecute = function(url){
	if (url == null || url == "") {
		if (this.file){
...
jMindMapNode.prototype.setNote = function(txt){
	if(!jMap.saveAction.isAlive()) {
		return null;
	}
...
jMindMapNode.prototype.setNoteExecute = function(txt){
	if (txt == null || txt == "") {
		if (this.note){
...
jMindMapNode.prototype.setHasMoodleExecute = function(){
	this.moodleIcon = RAPHAEL.image(jMap.cfg.contextPath+'/menu/icons/moodle2.png',0, 0, 18, 18);
	if(Raphael.svg) this.groupEl.appendChild(this.moodleIcon.node);
	if(Raphael.vml) this.groupEl.appendChild(this.moodleIcon.Group);
}

jMindMapNode.prototype.setScoreLisExecute = function(val){
...
jMindMapNode.prototype.fix_flash = function(el) {
	...
	var emLength = embeds.length;
	for (i = 0; i < emLength; i++) {
		embed = embeds[i];
		var new_embed;
...
jMindMapNode.prototype.toXML = function(alone) {
	...
	if (this.userNo != null) {
		buf.add("USERNO=\"" + this.userNo + "\"");
	}
	
	if (this.userfullname != null) {
		buf.add("USERFULLNAME=\"" + this.userfullname + "\"");
	}
	
	if (this.userportrait != null) {
		buf.add("USERPORTRAIT=\"" + this.userportrait + "\"");
	}
	...
	if(fontSize != null) {
		buffer.add("<font size=\"" + this.getFontSize() + "\"/>");
	}
	
	//edge
	if(this.edge) {
		//<edge COLOR="#ff66e5" STYLE=“jRect” WIDTH="4"/>
		buffer.add("<edge color=\"" + this.edge.color + "\" width=\"" + this.edge.width + "\"/>");
	}
	
	
	//branch
	if(this.branch) {
		if(!this.branch.style) {
			if(this.connection != null) {
				this.branch.style = this.connection.type;
			} else {
				this.branch.style = "";
			}
		}
		buffer.add("<branch color=\"" + this.branch.color + "\" width=\"" + this.branch.width + "\" style=\"" + this.branch.style + "\" />");
	}
...
//				richcontent.add("</p>\n");
	...
	if(this.arrowlinks.length > 0) {
		var linksLength = this.arrowlinks.length;

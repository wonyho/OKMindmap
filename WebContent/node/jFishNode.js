...
jFishNode.prototype.updateNodeShapesPos = function(){
	...
	if(this.isVertical()) {
		// 첨부들 위치
		var attach_x = this.getLocation().x + this.getSize().height;
		if (note) {
			attach_x = attach_x - note.getBBox().width - 3;
			var note_y = this.getLocation().y + (this.getSize().width - note.getBBox().height) / 2;
			note && note.attr({x: attach_x, y: note_y});
		}

		if (file) {
			attach_x = attach_x - file.getBBox().width - 3;
			var file_y = this.getLocation().y + (this.getSize().width - file.getBBox().height) / 2;		
			file && file.attr({x: attach_x, y: file_y});
		}

		if (hyperlink) {
			attach_x = attach_x - hyperlink.getBBox().width - 3;
			var hyper_y = this.getLocation().y + (this.getSize().width - hyperlink.getBBox().height) / 2;		
...
jFishNode.prototype.blur = function(){
	...
	if (selectedNodes.contains(this)) {
		...
		this.connection && this.connection.line.animate({fill: this.background_color,
								stroke: this.branch.color || '#000000',
								"stroke-width": this.branch.width || '1'}, 300);
	}

...
jSunburstNodeController.prototype.dblclick = function(e) {
	...
	if(jMap.getSelected()) jMap.getSelected().zoomExecute();
};
...
jSunburstNodeController.prototype.mouseleave = function(e) {
	...
	if ($(el).closest('.jpopover').parent().attr(id) != this.node.popover.container.attr(id)) {
		...
		changeNodeGifFormat(this.node, false);
	}
};
...
var custom_sunburst_touchtime = 0;
jSunburstNode.prototype.addEventController = function(c) {
	...
	// 	if (custom_sunburst_touchtime == 0) {
	// 		// set first click
	// 		custom_sunburst_touchtime = new Date().getTime();
	// 	} else {
	// 		// compare first click to this click and see if they occurred within double click threshold
	// 		if (((new Date().getTime()) - custom_sunburst_touchtime) < 400) {
	// 			custom_sunburst_touchtime = 0;
...
jSunburstNode.prototype.setFile = function(url){
...
jSunburstNode.prototype.setFileExecute = function(url){
	if (url == null || url == "") {
		if (this.file){
...
jSunburstNode.prototype.setNote = function(txt){
	if(!jMap.saveAction.isAlive()) {
		return null;
	}
...
jSunburstNode.prototype.setNoteExecute = function(txt){
	if (txt == null || txt == "") {
		if (this.note){
	...
	var $img = $('<img />')
		...
		.load(function() {
			...
			if (node.img) {
				...
				node.popover.img
					.attr('src', this.src)
					.attr("onclick", "changeNodeGifFormat(null, true, this)")
					.width(imgSize.width)
					.height(imgSize.height);
			} else {
...
jSunburstNode.prototype.showPopover = function() {
	...
	// hover 시 gif 경로 변경
	/*if(this.popover.container.find("img").attr("src") != undefined){
		if(checkNodeImgFormat(this.popover.container.find("img").attr("src")) == "gif"){
			var gifSrc = this.popover.container.find("img").attr("src");
			gifSrc = gifSrc.replace("/map/file/", "/map/gif/");
			this.popover.container.find("img").attr("src", gifSrc);
		}
	}*/
};
...
jSunburstNode.prototype.computedTextExecute = function() {
	...
	// var fontSize = parseFloat(jMap.cfg.nodeFontSizes[jMap.cfg.nodeFontSizes.length-1]);
	var fontSize = node.fontSize;
	// var maxSize = parseFloat(jMap.cfg.nodeFontSizes[0]);
	var textLimit = Math.floor((width - 2 * jMap.layoutManager.padding) / (fontSize * 0.6));

	// for(var size = fontSize; size <= maxSize; size++) {
	// 	var limit = Math.floor((width - 2 * jMap.layoutManager.padding) / (size * 0.75)) - 2;
	...
	textLimit = Math.max(0, textLimit);

	if (node.plainText.length > textLimit) {
		var text = node.plainText.slice(0, Math.max(0, textLimit - 3));
		this.text.attr({
			text : text + '...'
		});
	}else{
	...
	// node.setFontSizeExecute(fontSize);
...
jSunburstNode.prototype.getLocation = function(){
	var popover = this.getPopoverLocation();
	var center = jMap.getLayoutManager().getCenterLocation();
	return {x:center.x - popover.top, y: center.y - popover.left};
}

jSunburstNode.prototype.setFontSizeExecute = function(size) {
	//
};
jSunburstNode.prototype.setEdgeColorExecute = function(color, width){
	if (color) {

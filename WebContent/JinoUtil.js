...
JinoUtil = (function () {				
	...
	Util.waitingDialog = function(str, type) {
		if($("#waitingDialog").hasClass('show')) return;

		var htmlContent = '';
		switch (type) {
			default:
				htmlContent += '<div class="d-block mx-auto my-3 default-loading-icon"><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div>';
				break;
		}
		htmlContent += (str || '잠시만 기다려주세요...');
		$("#waitingDialog").find('.modal-body').html(htmlContent);
		__$('#waitingDialog').modal('show');
	}
	Util.waitingDialogClose = function(){
		__$('#waitingDialog').modal('hide');
	}

	Util.showDialog = function(options) {
		if(jMap.cfg.default_menu_opacity == 'false') __$('.jino-app').removeClass('menu-opacity-active');
		if(__$('#iframeDialog').is(':visible') )  this.closeDialog();

		options = options || {};

		__$('#iframeDialog').find('.modal-body').removeClass('p-0');
		__$('#iframeDialog').find('.modal-dialog').attr('class', 'modal-dialog');
		__$('#iframeDialog').find('.modal-header').removeClass('bg-primary text-white');
		__$('#iframeDialog').find('.modal-title').html('');
		
		if(options.url) {
			__$('#iframeDialog').find('iframe').data('src', options.url);
		}

		if(options.size) {
			__$('#iframeDialog').find('.modal-dialog').addClass(options.size);
		}

		if(options.title) {
			__$('#iframeDialog').find('.modal-header').addClass('bg-primary text-white');
			__$('#iframeDialog').find('.modal-title').html(options.title);
		}

		if(options.nopadding) {
			__$('#iframeDialog').find('.modal-body').addClass('p-0');
		}

		__$('#iframeDialog').modal(options);
	}
	Util.closeDialog = function() {
		if(jMap.cfg.default_menu_opacity == 'false') __$('.jino-app').removeClass('menu-opacity-active');
		__$('#iframeDialog').modal('hide');
	...
	Util.toast = function(options) {
		...
		var toast = $('<div class="m-2 toast hide pointer-events-auto bg-'+(options.type || 'dark')+' text-'+(options.type == undefined || options.type == 'danger' ? 'white' : 'dark')+'" role="alert" aria-live="polite" aria-atomic="true" data-delay="5000">'
						+ '<div class="toast-header">'
						+ 	'<img src="' +(jMap.cfg.contextPath)+'/theme/dist/images/icons/toast-'+(options.type || 'default')+'-icon.svg" class="mr-auto" width="20px">'
						+ 	'<button type="button" class="ml-2 mb-1 close" data-dismiss="toast" aria-label="Close"><span aria-hidden="true">&times;</span></button>'
						+ '</div>'
						+ '<div class="toast-body">'+options.text+'</div>'
					+'</div>').appendTo(toastWrap);

		__$(toast).toast('show');
		__$(toast).on('hidden.bs.toast', function () {
			$(toast).remove();
		})
	};
	...
	Util.BookmarkCallback = function(xml){
		var createBookmarkNode = function (node, bookmarksData){
	    	for(var i = 0; i < bookmarksData.length; i++){
	    		if(bookmarksData[i].name != null && bookmarksData[i].name != "^^"){
	    			var param = {parent: node,
	    					text: convertHexNCR2Char(bookmarksData[i].name)};
	    			var newNode = jMap.createNodeWithCtrl(param, null, true);
	    			if(bookmarksData[i].location != null && bookmarksData[i].location != "^^"){
	    				newNode.setHyperlink(convertHexNCR2Char(bookmarksData[i].location));
	    			}
	    			else if(bookmarksData[i].children && bookmarksData[i].children.length > 0){
	    				createBookmarkNode(newNode, bookmarksData[i].children)
	    			}
	    		}
	    		...
	    		if(bookmarksData[i].children && bookmarksData[i].children.length > 0 &&
	    				bookmarksData[i].name != null && bookmarksData[i].name != "^^"){
	    			var param = {parent: node,
	    					text: convertHexNCR2Char(bookmarksData[i].name)};
		...
		//csedung catch too long bookmark
		// Allow file less than 30000 character
		if(xml.length < 30000){
    		...
    		alert('Out of memory: bookmark too long.');
    		// $("#dialog_c").dialog("close");
    		return;
    	}	
	}

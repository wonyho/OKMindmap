...
var removeGradient = function(node){
	...
	if(node.type == 'jFishNode') {
		node.body.attr({fill: node.background_color, stroke: node.edge.color, opacity: 0, 'fill-opacity': 0});
	} else {
...
//			var nodeLength = srcNodes.length;
//			for (var i = 0; i < nodeLength; i++) {
//				jMap.controller.unfoldAllChild(srcNodes[i]);
...
//				}
//			}
//			jMap.controller.unfoldAllChild(targNode);
...
//			jMap.layoutManager.layout(true);
			...
			if(initFoldingTargetNode.isRootNode()){
				jMap.initFolding(initFoldingTargetNode);
				jMap.layoutManager.updateTreeHeightsAndRelativeYOfDescendantsAndAncestors(initFoldingTargetNode);

				var nodes = initFoldingTargetNode.getChildren();
				if(nodes.length > 0){
				...
				if(initFoldingTargetNode.folded){
					initFoldingTargetNode.setFolding(false);
					initFoldingTargetNode.setFolding(true);
				}else{
					jMap.initFolding(initFoldingTargetNode);
					initFoldingTargetNode.setFolding(true);
					initFoldingTargetNode.setFolding(false);
	...
	if(jMap.getSelected().arrowlinks.length < 1){
		$("#menusettingArrowLink").removeClass("show");
	}else{
		$("#menusettingArrowLink").addClass("show");
	}
}
...
jNodeController.prototype.mousemove = function(e){
	...
	if(!jMap.positionChangeNodes && this.node.isFoldingHit(e) && !this.node.isRootNode() && !this.node.getChildren().isEmpty()) {
		...
		if(this.node.isLeft())
			...
			coords = ' 0 7';
		d.body.style.cursor = "url('"+jMap.cfg.contextPath+"/images/folding_blue.png')"+coords+",default";
...
jNodeController.prototype.touchmove = function(e){
...
//		originalEvent.returnValue= false;
	...
	e.offsetX = e.clientX - rect.left;
	e.offsetY = e.clientY - rect.top;
	$("#nodePopupTipBox").css("top", rect.left + rect.height + 10 + "px");
	$("#nodePopupTipBox").css("left", rect.left  + "px");
	$("#nodePopupTipBox").css("display", "inline-block");
	...
	if (this.node.isFoldingHit(e) && !this.node.isRootNode() && !this.node.getChildren().isEmpty() && targ.nodeName != "image" && !(ISMOBILE || supportsTouch)) {
		// Folding & unFolding
		jMap.controller.foldingAction(this.node);
	} else if(isDoubleClick) {
		this.node.controller.dblclick(e);
	}
	// ======
	...
	// 	// ** 하이퍼링크를 클릭시에는 폴딩되지 말아야 한다.
	// 	// ** 그래서 image를 검사한다. (hyperlink는 그림을 클릭하므로..)
	// 	if (this.node.isFoldingHit(e) && targ.nodeName != "image" && !(ISMOBILE || supportsTouch)) {
	// 		// Folding & unFolding
	// 		jMap.controller.foldingAction(this.node);		
	// 	}
	// }
...
jNodeController.prototype.dblclick = function(e){
	...
	if (originalEvent.preventDefault)
		...
		originalEvent.returnValue= false;
	$("#nodePopupTipBox").css("display", "none");
	//jMap.nodePopupTipBox[0].style.display = "none";
	// if(!(ISMOBILE || supportsTouch)){
...
jNodeController.prototype.drop = function(e){
	...
	if(droptext.substr(0,7) == "http://" || droptext.substr(0,8) == "https://"){

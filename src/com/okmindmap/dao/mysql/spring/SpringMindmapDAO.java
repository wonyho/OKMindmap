...
public class SpringMindmapDAO extends SpringDAOBase implements MindmapDAO {
	...
	public int insertNode(Node node, int map_id, int parent_id) {
		...
		synchronized (lock) {
			...
			id = getJdbcTemplate().queryForObject(queryInsert, new Object[]{
					map_id, node.getBackgroundColor(), node.getColor(), node.getFolded(), identity,
					node.getLink(), node.getFile(), node.getNote(), node.getPosition(), node.getStyle(), node.getText(),
...
//		}
	...
	public int insertArrowLink(ArrowLink link, int node_id, int map_id) {
		String query = "INSERT INTO mm_arrowlink (id, node_id, color, destination,"
				+ " endarrow, endinclination, identity, startarrow, startinclination, map_id, text, style, width)"
				+ " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	...
	public int updateNode(Node node, int mapId) throws DataAccessException {
		...
		String sql = "UPDATE mm_node SET  map_id = ?,"
				...
				+ " identity = ?, link = ?, file = ?, note = ?, position = ?,"
				+ " style = ?, text = ?,"
		...
		synchronized (lock) {
			...
			result = getJdbcTemplate().update(
					...
					new Object[] { node.getMapId(),
							...
							identity, node.getLink(), node.getFile(), node.getNote(), node.getPosition(),
							node.getStyle(), StringEscapeUtils.unescapeHtml(node.getText()),
	...
	public int updateArrowLink(ArrowLink link) throws DataAccessException {
		String sql = "UPDATE mm_arrowlink SET " + " color = ?,"
				...
				+ " startarrow = ?," + " startinclination = ?, text=?, width = ?, style = ? "
				+ " WHERE id = ?";
		...
		return getJdbcTemplate().update(
				...
				new Object[] { link.getColor(), link.getDestination(),
						...
						link.getStartArrow(), link.getStartInclination(),
						link.getText(), link.getWidth(), link.getStyle(), 
						link.getId() });
	}
...
//		Node node = getNode(id);
		...
		synchronized (lock) {
			...
			result = getJdbcTemplate().queryForObject(sql, new Object[]{id, map_id}, Integer.class);
			/*
			// 자식과 자식노드를 전부 지운다.
...
//	}
	...
	@SuppressWarnings("unchecked")
	public List<Attribute> getAttributes(String name, String value)
			throws DataAccessException {
		String sql = "SELECT * FROM mm_attribute WHERE name = ? AND value = ? ORDER BY map_id";

		return getJdbcTemplate().query(sql, new Object[] { name, value },
				new AttributeRowMapper());
	}
...
//		Node oldParent = getNode(node.getParentId());
		...
		*/
		...
		Object lock = LockObjectManager.getInstance().lock(key);
		synchronized (lock) {
			result = getJdbcTemplate().queryForObject(sql,
					new Object[]{node_id, newParent.getId(), node.getMapId()}, Integer.class);
		}
		LockObjectManager.getInstance().unlock(key);
	...
	public int moveNodeBeforeSibling(int node_id, int next_id, int parent_id, int map_id) {
		...
		Object lock = LockObjectManager.getInstance().lock(key);
		synchronized (lock) {
			result = getJdbcTemplate().queryForObject(queryInsert, new Object[]{
					node_id,
					next_id,
					parent_id,
					map_id
	...
	public int countAllMaps(String searchfield, String search) throws DataAccessException {
		...
		if (search != null && search.trim().length() > 0) {
			...
			return getJdbcTemplate().queryForObject(sql, new Object[] { search.trim() }, Integer.class);
		} else {
	...
	public User getMapOwner(int mapid) {
		...
		String sql = "SELECT m.id AS mapid" +
				", CASE WHEN o.userid IS NOT NULL THEN u.auth ELSE 'manual' END AS auth" +
...
//			" LEFT JOIN (SELECT map_id, COUNT(id) AS revisioncnt FROM mm_map_timeline GROUP BY map_id) rev ON m.id = rev.map_id ";
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<Map> getUserMaps(int userId, Long createdFrom, Long createdTo) throws DataAccessException {
		// slq 쿼리문 변경시 getUserMaps 오버로딩된 함수것도 같이 변경할것
		String sql = "SELECT m.* FROM mm_map m"
				+ " JOIN mm_map_owner o ON m.id = o.mapid"
				+ " WHERE o.userid = ? AND m.created >= ? AND m.created <= ? "
				+ " ORDER BY name ";

		return getJdbcTemplate().query(sql, new Object[] { userId, createdFrom, createdTo },
				new MapRowMapper());
	}
	...
	public List<Map> getUserMaps(int userid, int page, int pagelimit,
		...
		if(searchfield == null) {
			...
			sql = "SELECT DISTINCT f.map_id, m.* "
					+ " FROM mm_map m"
					+ " JOIN mm_map_owner o ON m.id = o.mapid"
					+ " JOIN mm_node f ON m.id = f.map_id"
					+ " LEFT JOIN mm_foreignobject n ON m.id = n.map_id"
//					+ " LEFT JOIN mm_richcontent mr ON m.id = mr.map_id"
					+ " WHERE o.userid = ?";
			if (search != null && search.trim().length() > 0) {
				sql += " AND (f.text LIKE CONCAT('%', ?, '%')"
						+ " OR n.content LIKE CONCAT('%', ?, '%'))";
//						+ " OR mr.content LIKE CONCAT('%', ?, '%'))";
				params.add(search.trim());
				params.add(search.trim());
//				params.add(search.trim());
			}
...
//			" WHERE  mm_group_member.userid = ?  AND  m.id IS NOT NULL AND  mm_group_member.STATUS = 1";
		...
		String sql = "";
		if(searchfield == null) {
			sql = "SELECT mm_share.id id, mm_group.name AS group_name, m.id map_id, m.name map_name, m.created, m.map_key, m.viewcount, mm_user.id user_id, mm_user.lastname user_lastname, mm_user.firstname user_firstname, mm_user.username user_username, mm_user.email user_email   FROM mm_group_member "+ 
					" LEFT JOIN mm_group ON mm_group_member.groupid = mm_group.id 	 "+
					" LEFT JOIN  mm_share_group ON mm_group.id = mm_share_group.groupid 	 "+
					" LEFT JOIN mm_share ON mm_share_group.shareid = mm_share.id 	 "+
					" LEFT JOIN mm_map m ON mm_share.mapid= m.id "+
			...
			sql = "SELECT DISTINCT m.id map_id, mm_share.id id, mm_group.name AS group_name, m.name map_name, m.created, m.map_key, m.viewcount, mm_user.id user_id, mm_user.lastname user_lastname, mm_user.firstname user_firstname, mm_user.username user_username, mm_user.email user_email   FROM mm_group_member "+ 
					" LEFT JOIN mm_group ON mm_group_member.groupid = mm_group.id 	 "+
					" LEFT JOIN  mm_share_group ON mm_group.id = mm_share_group.groupid 	 "+
					" LEFT JOIN mm_share ON mm_share_group.shareid = mm_share.id 	 "+
					" LEFT JOIN mm_map m ON mm_share.mapid= m.id "+
		...
		*/
		...
		if(searchfield == null) {
			...
			sql = "SELECT m.* "
					...
					+ "LEFT JOIN mm_user u ON u.id = o.userid "
					+ "LEFT JOIN mm_map_owner_info oi ON oi.mapid = m.id "
					+ " LEFT JOIN mm_node f ON m.id = f.map_id"
					+ " LEFT JOIN mm_foreignobject n ON m.id = n.map_id"
					+ " LEFT JOIN mm_richcontent mr ON m.id = mr.map_id";
...
//		}
	...
	public int insertSlide(int nodeId, double x, double y, double scalex, double scaley, double rotate, int showdepths) throws DataAccessException {
		int count = getJdbcTemplate().queryForObject("SELECT count(*) FROM mm_presentation_slide WHERE node_id = ? ",
				new Object[]{nodeId}, Integer.class);
		if(count > 0) {
	...
	public int countDuplicateMapName(int userid, String mapName) {
		...
		return getJdbcTemplate().queryForObject(sql, new Object[] { userid, mapName }, Integer.class);
	}
	@Override
	...
	public List<java.util.Map<String, Object>> getStatisticsByMonth(int userid) throws DataAccessException {
		// created: Unix Millisecond Timestamp	
//		String sql = "SELECT DATE_FORMAT(FROM_UNIXTIME(mm_map.created/1000), '%m/%y') AS  month, "
//				+ "COUNT(*) AS total FROM mm_map INNER JOIN mm_map_owner ON mm_map.id = mm_map_owner.mapid "
//				+ "WHERE userid = ? GROUP BY DATE_FORMAT(FROM_UNIXTIME(mm_map.created/1000), '%Y %m')";
		
		String sql = "SELECT DATE_FORMAT(FROM_UNIXTIME(mm_map.created/1000), '%m/%y') AS  month, "
				+ "COUNT(*) AS total FROM mm_map INNER JOIN mm_map_owner ON mm_map.id = mm_map_owner.mapid "
				+ "WHERE userid = ? GROUP BY DATE_FORMAT(FROM_UNIXTIME(mm_map.created/1000), '%m/%y')";
		return getJdbcTemplate().queryForList(sql,new Object[] { userid});
	}
	
	public List<java.util.Map<String, Object>> getStatisticsByMonth(int userid, Long createdFrom, Long createdTo) throws DataAccessException {
		// created: Unix Millisecond Timestamp	
		String sql = "SELECT DATE_FORMAT(FROM_UNIXTIME(mm_map.created/1000), '%m/%y') AS  month, COUNT(*) AS total FROM mm_map INNER JOIN mm_map_owner ON mm_map.id = mm_map_owner.mapid WHERE userid = ? AND mm_map.created >= ? AND mm_map.created <= ?  GROUP BY DATE_FORMAT(FROM_UNIXTIME(mm_map.created/1000), '%Y %m')";
		return getJdbcTemplate().queryForList(sql,new Object[] { userid, createdFrom, createdTo});
	}

	public boolean canDeleteFileContent(String filePath, int mapId) {
		String sql = "SELECT COUNT(*) FROM `mm_richcontent` WHERE content LIKE \"%"+filePath+"%\" AND map_id= ?";
		int rows = getJdbcTemplate().queryForObject(sql, 
				new Object[] { mapId }, 
				Integer.class);
		return rows < 2;
	}
	
	public boolean canDeleteFileAttached(String filePath, int mapId) {
		String sql = "SELECT COUNT(*) FROM `mm_node` WHERE file LIKE \"%"+filePath+"%\" AND map_id= ?";
		int rows = getJdbcTemplate().queryForObject(sql, 
				new Object[] { mapId }, 
				Integer.class);
		return rows < 2;
	...
	@SuppressWarnings("unchecked")
	public List<Node> searchNodes(int mapId, String keyword, boolean hidden){
		String sql = "SELECT * FROM `mm_node` WHERE map_id = " + mapId + " AND text LIKE '%" + keyword + "%'";
//		if(!hidden) sql +=  " AND folded = 'false'";
	
		return getJdbcTemplate().query(sql, 
				new Object[]{},
				new SearchNodeRowMapper());
	}
}

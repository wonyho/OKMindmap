...
public class ExportPersionalPpt implements Export {
...
//	Logger logger; // wait for the upgrade to log4j 2.15.0
	...
	public ExportPersionalPpt() {
//		logger = Logger.getLogger(ExportPpt_old.class); // wait for the upgrade to log4j 2.15.0
	}

	public ExportPersionalPpt(MindmapService mindmapService) {
...
//		logger = Logger.getLogger(ExportPpt_old.class); // wait for the upgrade to log4j 2.15.0
	...
	public void transform(Map map, Properties prop, OutputStream out) throws IOException {
		...
		this.createSlideShow(this.realPath + "../okmindmap_upload"+"/slide_master/" + this.themeSource);
	...
	private void createSlideShow(String theme) {
		try {
			...
			int j = this.slideShow.getSlides().size();
			for(int i = 0; i< j; i++)
	...
	private void saveSlideShow(OutputStream out) {
		if(this.slideLayout.equals(this.slideMaster.getLayout(SlideLayout.TITLE_AND_CONTENT))) {
			if(this.slide.getPlaceholder(1).getText().length() < 2) {
				this.slide.removeShape(this.slide.getPlaceholder(1));
			}
		}
	...
	private void putTextIntoSlide() {
		...
		for (Text t : this.textContents) {
			if (t.hasUrl()) {
				this.addHyperlink(t.getText(), t.getUrl(), t.getIdentLevel());
			} else {
				this.addTextBullet(t.getText(), t.getIdentLevel());
			}
		}

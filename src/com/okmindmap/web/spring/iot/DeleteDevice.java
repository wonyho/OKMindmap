...
public class DeleteDevice extends BaseAction {
		...
		if (user != null) {
			...
			if("1".equals(confirm)) {
				if(user.getId() != 2) {
					...
					IotDevices device = this.iotService.getDevice(user.getId(), Integer.parseInt(id));
					if(device.getUserId() == user.getId()) {
						this.iotService.deleteDevice(Integer.parseInt(id));
					}
			...
			return new ModelAndView("iot/deleteDevice", "data", data);
		}
		return null;
	}
